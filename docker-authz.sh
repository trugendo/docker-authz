...
help(){
    cat<<EOF
Use this command to handle docker-authz plugin easily
-----------------------------------------------------
-> docker-authz -r|--reload
        - Reload docker and docker-authz at the same time
-> docker-authz -q|--query state|port
        - Get the current plugin state and port details
-> docker-authz -t|--toggle
        - Enable/disable the plugin
-> docker-authz -s|--set-port <port number>
...
daemon_reload(){
    local x xx ret=0
    msg BLUE "restarting docker & docker-authz"
    for x in stop start; do
        for xx in docker-authz docker; do
            [[ $xx == "docker-authz" ]] && opt="--systemd" || opt=""
            daemon -a $x -s $xx $opt || {
                let ret+=$?
                warn "$xx $x failed"
            }
