...
def req():
    ...
    if search(r'/(exec)$', res["RequestUri"]) != None:
        ...
        if match(r'^titus$|^elly$', dd["User"])!=None:
            response={"Allow":True}
        else:
            response={"Allow":False, "Msg":"You are not authorized to Run Execute command"}
    if search(r'/(run)$', res["RequestUri"]) !=None:
        dd = json.loads(base64.b64decode(res["RequestBody"]))
        if match(r'^rugendo$|^elly$', dd["User"]) !=None:
            response={"Allow":True}
        else:
            response = {"Allow": False, "Msg": "You are not authorized to Run any Containers!"}
    if not enabled:
        response={"Allow":True}
